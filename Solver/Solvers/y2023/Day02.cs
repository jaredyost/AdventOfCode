...
namespace AdventOfCode.Solvers.y2023
{
    ...
    {
        public Day02() : base(new DateOnly(2023, 12, 02)) { }

        public override string SolvePart1(string[] aInput)
        {
            return Game.ParseGames(aInput).Where(x => x.Red <= 12 && x.Green <= 13 && x.Blue <= 14).Sum(x => x.ID).ToString();
        }

        public override string SolvePart2(string[] aInput)
        {
            return Game.ParseGames(aInput).Sum(x => x.Red * x.Green * x.Blue).ToString();
        }
