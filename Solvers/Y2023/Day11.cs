...
{
    public class Day11 : BaseDay2023
    {
        protected override int Day => 11;

        public override ValueTask<string> SolvePart1(string[] aInput)
        {
            return new(SumShortestPaths(aInput, 2).ToString());
        }

        public override ValueTask<string> SolvePart2(string[] aInput)
        {
            return new(SumShortestPaths(aInput, 1000000).ToString());
        }
        ...
        private static ulong SumShortestPaths(string[] aInput, int aGrowthFactor)
        {
            ...
            for (int i = 0; i < galaxies.Count - 1; i++)
            {
                for (int j = i + 1; j < galaxies.Count; j++)
                {
                    ...
                    _ = int.Min(galaxies[i].X, galaxies[j].X);

                    int startY = int.Min(galaxies[i].Y, galaxies[j].Y);

                    _ = int.Min(galaxies[i].Y, galaxies[j].Y);
